...
def dict_to_dataframe(dict: dict) -> pd.DataFrame:
...
def get_column(df: pd.DataFrame, column_name: str) -> pd.Series:
    new_df = df.copy()
...
def get_top_two(df: pd.DataFrame) -> pd.DataFrame:
    new_df = df.copy()
...
def population_density(df: pd.DataFrame) -> pd.DataFrame:
    new_df = df.copy()
